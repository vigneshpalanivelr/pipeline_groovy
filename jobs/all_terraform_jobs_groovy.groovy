...
pipelineJob('tf-1-rds-db-build-generic-job') {
        ...
        definition {
                cps {
                        script(readFileFromWorkspace('pipeline/tf-1-rds-db-build-generic.groovy'))
                        sandbox()
                }
        }
}

// RDS Cluster Build Generic Job
pipelineJob('tf-1-rds-cluster-build-generic-job') {
        description('Explains how to use Jenins Approval for Build Jobs')
        logRotator(-1,-1)
        parameters{
                choiceParam('gitRepo'                   , [terraformRepo]       	, '')
        ...
        definition {
                cps {
                        script(readFileFromWorkspace('pipeline/tf-1-rds-cluster-build-generic.groovy'))
                        sandbox()
                }
        }
}
