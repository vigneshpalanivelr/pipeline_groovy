...
def terraform_plan(global_tfvars,sns_tfvars) {
	sh "terraform plan -no-color -out=tfplan -input=false -var-file=${global_tfvars} -var-file=${sns_tfvars}"
}
...
def terraform_plan_destroy(global_tfvars,sns_tfvars) {
    sh "terraform plan -destroy -no-color -out=tfdestroy -input=false -var-file=${global_tfvars} -var-file=${sns_tfvars}"
}
